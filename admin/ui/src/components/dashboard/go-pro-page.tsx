  ...
  return (
    <div className="jwt-max-w-4xl jwt-mx-auto jwt-space-y-8">
      ...
      <Card className="jwt-bg-slate-800 jwt-text-white jwt-overflow-hidden jwt-text-center jwt-p-8 jwt-rounded-xl">
        <Badge variant="secondary" className="jwt-bg-amber-400 jwt-text-slate-900 jwt-font-bold jwt-text-sm jwt-mb-4">
        ...
        <div className="jwt-flex jwt-items-center jwt-justify-center jwt-space-x-4 jwt-mt-4">
          <span className="jwt-text-4xl jwt-font-bold jwt-text-white">${(59.99 * (1 - discount / 100)).toFixed(2)}/year</span>
          <span className="jwt-text-2xl jwt-text-slate-400 jwt-line-through">$59.99</span>
        </div>
        <Button size="lg" className="jwt-bg-emerald-600 hover:jwt-bg-emerald-700 jwt-text-white jwt-text-lg jwt-font-bold jwt-mt-6 jwt-px-8 jwt-py-6">
          <Sparkles className="jwt-mr-2 jwt-h-5 jwt-w-5" />
          Claim Your {discount}% Discount
        </Button>
        <p className="jwt-text-xs jwt-text-slate-400 jwt-mt-2">30-day money-back guarantee. Active in 2 minutes.</p>
      </Card>
